...
!      -- this is written with the goal of ZERO branching statements
...
!      -- N O  M A S K --
       ...
       if (.not. sfc_flux(n)%pblmix) then ! Non-PBL may be positive or negative (e.g. OCN, NEP)
          k = params%km ! at the surface
          if (sfc_flux(n)%diurnal) then
             inst%prod(:,:,k)   = inst%prod(:,:,k) +  &
                                  sfc_flux(n)%flux(:,:) * fDNL(:,:) * &
                                  sfc_flux(n)%scalefactor * grav / met%delp(:,:,k) * &
                                  params%airmw / inst%mw
          endif
          if (.not. sfc_flux(n)%diurnal) then
...
!      -- N O  M A S K --
       if (sfc_flux(n)%pblmix) then
          ...
          if (sfc_flux(n)%diurnal) then
             ...
             inst%prod(:,:,k)   = inst%prod(:,:,k) +  &
                                  sfc_flux(n)%flux(:,:) * sfc_flux(n)%scalefactor * grav / met%delp(:,:,k) * &
                                  params%airmw / inst%mw
          endif
       endif
       else
